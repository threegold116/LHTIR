...
-解耦process和answer
1. 增加额外的信息
因为总是把reward放到turn的最后一个token，因此通过额外的answer_index来确定answer的reward
'''
def compute_process_KM_with_answer_index(response: str, codes: dict, unsolved_set: dict, solve_rate: float, split: str, answer: str = None, gt_tool_call: list = None, tokenizer=None, valid_response_ids=None):
    if split == "train":
        if isinstance(gt_tool_call, str):
            gt_tool_call = json.loads(gt_tool_call)
        scores = [0.0] * len(valid_response_ids)
        ...
        for i, turn in enumerate(turns): #turn-level reward
            ...
            scores[end - 1] = process_reward[i]
            #------THREEGOLDCHANGE--------#
            if i == answer_turn_index:
                answer_index = end - 1
            #------THREEGOLDCHANGE--------#

        return {"score": scores, "answer_index": answer_index}
    elif split == "test":  
        return compute_solve_f1(response, codes, unsolved_set, solve_rate, split, answer, gt_tool_call, tokenizer, valid_response_ids)
