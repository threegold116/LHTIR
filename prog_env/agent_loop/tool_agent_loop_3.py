...
class ToolAgentLoop_3(AgentLoopBase):
    ...
    async def run(
        ...
        while True:
            ...
            assistant_token_start = len(response_mask)
            prompt_ids += response_ids
            response_mask += [1] * len(response_ids)
            if response_ids:
                assistant_token_spans.append((assistant_token_start, len(response_mask) - 1))
            ...
            if len(response_mask) + len(tool_response_ids) >= self.response_length:
                step_complete_list.append(False)
                stop_reason = "tool_response_overflow"
                break
        ...
        void_turn_list = [not step_complete for step_complete in step_complete_list]
        has_void_turn = any(void_turn_list)
        overlong_limit_reached = stop_reason in {
        ...
        if self.enable_overlong_mask and overlong_limit_reached:
            ...
            elif self.overlong_mask_scope in {"tail", "last_assistant_turn"} and stop_reason in {"trunctated_step", "response_length_limit"}:
                #FIXME:应该只有在truncated_step(当前步超过step_length)或者response_length_limit(当前步+context超过response_length，会被截断)时才进行last_assistant_turn的mask
                if assistant_token_spans:
                    start, end = assistant_token_spans[-1]
                    for idx in range(start, end + 1):
                        response_mask[idx] = 0
                    overlong_mask_applied = True
            else:
                logger.warning(
                    f"Unknown overlong_mask_scope: {self.overlong_mask_scope}, fallback to no mask application."
                )
        #--------THREEGOLDCHANGE--------#
        '''
