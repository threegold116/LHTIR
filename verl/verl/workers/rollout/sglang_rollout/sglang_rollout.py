...
class SGLangRollout(BaseRollout):
    ...
    async def _handle_engine_generate(
        ...
        '''
        1.新增max_tokens的控制逻辑
        - 传入max_new_tokens和max_tokens的控制逻辑:之前的代码中默认采用min(self.config.response_length, self.config.max_model_len - len(generation_prompt_ids) - 1),现在增加可能的max_new_tokens限制(因为可能比这两个值都小)
        '''
        max_possible_tokens = self.config.max_model_len - len(generation_prompt_ids)

        if max_possible_tokens < 0:
            raise ValueError(
                f"Prompt length ({len(generation_prompt_ids)}) exceeds the model's maximum context length "
        ...
        if "max_new_tokens" in sampling_params:
            ...
            max_new_tokens = min(
                self.config.response_length, self.config.prompt_length + self.config.response_length - len(generation_prompt_ids) - 1
            )
        # max_new_tokens = min(self.config.response_length, self.config.max_model_len - len(generation_prompt_ids) - 1)
        max_new_tokens = min(max_new_tokens, self.config.response_length, self.config.max_model_len - len(generation_prompt_ids) - 1)
        #--------THREEGOLDCHANGE--------#
